  ...
  public static void statement(CommonTree ast, IRTree irt)
  {
    ...
    if (tt == WRITE) {
      ...
      if (type.equals("int")) {
        ...
        // irt.setOp("SEQ");
        irt.setOp("WriteB");
        IRTree child = recursive_bexp_eval(ast1, irt1);
        irt.addSub(child);
      ...
      */
        ...
        irt.addSub(childTwo);
        recursive_bexp_eval(opAST, childTwo);

        IRTree do_this_thing = new IRTree("Comp ");
        irt.addSub(do_this_thing);
        statements(ifPart, do_this_thing);

        IRTree do_this_thing2 = new IRTree("Comp ");
        irt.addSub(do_this_thing2);
        statements(elsePart, do_this_thing2);
...
// Needs to be tail recursive.
  public static IRTree recursive_bexp_eval(CommonTree ast, IRTree irt) {
    Token t = ast.getToken();
    int tt = t.getType();

    IRTree child_left  = new IRTree();
    IRTree child_right = new IRTree();

    // if(ast.getChild(0)) {
      if(tt == NOT) {
        
        irt.setOp("Neg");
        irt.addSub(child_left);
  
        child_left  = recursive_bexp_eval((CommonTree)ast.getChild(0), child_left);

      } else if(tt == LOGAND) {
        
        irt.setOp("And");
        irt.addSub(child_left);
        irt.addSub(child_right);
  
        child_left  = recursive_bexp_eval((CommonTree)ast.getChild(0), child_left);
        child_right = recursive_bexp_eval((CommonTree)ast.getChild(1), child_right);
  ...
  public static void expression(CommonTree ast, IRTree irt)
  {
    ...
    } else if (tt == PLUS || tt == MULTIPLY || tt == MINUS || tt == DIVIDE || tt == SMALLEQUAL || tt == EQUAL || tt == NOT || tt == LOGAND) {
