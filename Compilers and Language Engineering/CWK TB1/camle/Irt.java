  ...
  public static void statement(CommonTree ast, IRTree irt)
  {
    ...
    if (tt == WRITE) {
      ...
      */
        ...
        int startedLoopLabel = labelCount;
        IRTree startedLoopLabelTree = new IRTree("LABEL", new IRTree("N"+startedLoopLabel));
        labelCount++;
        ...
        IRTree seqRoot = new IRTree("SEQ");
        seqRoot.addSub(statementsOfIfPart);
        statements(ifPart, statementsOfIfPart);
        startedLoopParent.addSub(seqRoot);
...
// Translate has to replace both the while and the if conditionals.
  ...
  public static void expression(CommonTree ast, IRTree irt)
  {
    ...
    } else if (tt == PLUS || tt == MULTIPLY || tt == MINUS || tt == DIVIDE || tt == XOR) {
