...
rewards_array = [[0 for i in range(0, len(epsilon_vs))] for j in range(0, len(beta_vs))]
for b in range(0, len(beta_vs)):
    for e in range(0, len(epsilon_vs)):
        rewards = []
        for k in range(0, 30):
            (reward_vs, choices, VAs, VBs, TI) = model_simulate([epsilon_vs[e], beta_vs[b]], reward_probs)
...
plt.imshow(rewards_array, cmap='hot')
plt.colorbar()
plt.show()
