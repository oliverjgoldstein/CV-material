...
float loop_fusion(t_speed* cells, t_speed* tmp_cells, const t_param params, uchar* obstacles, const float c_sq, 
    ...
    for (int jj = 0; jj < params.nx; jj++) {
        ...
        if (!obstacles[accelerate_flow_row * params.nx + jj]
            && (cells[accelerate_flow_row * params.nx + jj].speeds[3] - accelerate_flow_w1) > 0.0
            && (cells[accelerate_flow_row * params.nx + jj].speeds[6] - accelerate_flow_w2) > 0.0
            && (cells[accelerate_flow_row * params.nx + jj].speeds[7] - accelerate_flow_w2) > 0.0) {

            /* increase 'east-side' densities */
            cells[accelerate_flow_row * params.nx + jj].speeds[1] += accelerate_flow_w1;
            cells[accelerate_flow_row * params.nx + jj].speeds[5] += accelerate_flow_w2;
            cells[accelerate_flow_row * params.nx + jj].speeds[8] += accelerate_flow_w2;
            /* decrease 'west-side' densities */
            cells[accelerate_flow_row * params.nx + jj].speeds[3] -= accelerate_flow_w1;
            cells[accelerate_flow_row * params.nx + jj].speeds[6] -= accelerate_flow_w2;
            cells[accelerate_flow_row * params.nx + jj].speeds[7] -= accelerate_flow_w2;
        }
    }
    ...
    #pragma loop count (256)
        for (int jj = 0; jj < params.nx; jj++) {
            ...
            float t0 = tmp_cells[ii * params.nx + jj].speeds[0];
            float t1 = tmp_cells[ii * params.nx + jj].speeds[1];
            float t2 = tmp_cells[ii * params.nx + jj].speeds[2];
            float t3 = tmp_cells[ii * params.nx + jj].speeds[3];
            float t4 = tmp_cells[ii * params.nx + jj].speeds[4];
            float t5 = tmp_cells[ii * params.nx + jj].speeds[5];
            float t6 = tmp_cells[ii * params.nx + jj].speeds[6];
            float t7 = tmp_cells[ii * params.nx + jj].speeds[7];
            float t8 = tmp_cells[ii * params.nx + jj].speeds[8];
            ...
            float x0 = (1-ob_val)*(t0 + params.omega * (d_equ[0] - t0)) + ob_val * t0;

            float x1 = (1-ob_val)*(t1 + params.omega * (d_equ[1] - t1)) + ob_val * (ob_val * t3 + (1-ob_val) * t1);
            
            float x2 = (1-ob_val)*(t2 + params.omega * (d_equ[2] - t2)) + ob_val * (ob_val * t4 + (1-ob_val) * t2);
            
            float x3 = (1-ob_val)*(t3 + params.omega * (d_equ[3] - t3)) + ob_val * (ob_val * t1 + (1-ob_val) * t3);
            
            float x4 = (1-ob_val)*(t4 + params.omega * (d_equ[4] - t4)) + ob_val * (ob_val * t2 + (1-ob_val) * t4);
            
            float x5 = (1-ob_val)*(t5 + params.omega * (d_equ[5] - t5)) + ob_val * (ob_val * t7 + (1-ob_val) * t5);
            
            float x6 = (1-ob_val)*(t6 + params.omega * (d_equ[6] - t6)) + ob_val * (ob_val * t8 + (1-ob_val) * t6);
            
            float x7 = (1-ob_val)*(t7 + params.omega * (d_equ[7] - t7)) + ob_val * (ob_val * t5 + (1-ob_val) * t7);
            
            float x8 = (1-ob_val)*(t8 + params.omega * (d_equ[8] - t8)) + ob_val * (ob_val * t6 + (1-ob_val) * t8);
            ...
            cells[ii*params.nx + jj].speeds[0] = x0;
            cells[ii*params.nx + jj].speeds[1] = x1;
            cells[ii*params.nx + jj].speeds[2] = x2;
            cells[ii*params.nx + jj].speeds[3] = x3;
            cells[ii*params.nx + jj].speeds[4] = x4;
            cells[ii*params.nx + jj].speeds[5] = x5;
            cells[ii*params.nx + jj].speeds[6] = x6;
            cells[ii*params.nx + jj].speeds[7] = x7;
            cells[ii*params.nx + jj].speeds[8] = x8;
        }
    }
