...
\begin{code}
  AllTproof : ∀ {A : Set} {P : A → Set} {xt : Tree A}
    → AllT (¬_ ∘ P) xt → ¬ (AnyT P xt)  
  AllTproof (leaf x₁) (leaf x₂) = x₁ x₂
  AllTproof (l branch l₁) (left p) = AllTproof l p
  AllTproof (l branch l₁) (right p) = AllTproof l₁ p
\end{code}
...
--begin
    ...
    ξ-letc : ∀ {Γ} {A B} {M M′ : Γ ⊢ Comp A} {N : Γ , A ⊢ Comp B}
      → M —→ M′
      → letc {Γ} {A} {B} M N —→ letc {Γ} {A} {B} M′ N 

    β-error : ∀ {Γ} {A} {B} {N : Γ , A ⊢ Comp B} {msg}  
         -----------------------
       → letc {Γ} {A} (error msg) N —→ error msg

    β-ok : ∀ {Γ} {A} {B} {V : Γ ⊢ A} {N : Γ , A ⊢ Comp B}
       → Value V
         ---------------------
       → letc {Γ} {A} (ok {Γ} {A} V) N —→ N [ V ]
...
\begin{code}
  ...
  inherit Γ (`tt) (`ℕ)        =  no  (λ())
  inherit Γ (ƛ x ⇒ x₁) ⊤      =  no  (λ())
  inherit Γ `zero ⊤           =  no  (λ())
  inherit Γ (`suc x) ⊤        =  no  (λ())
  inherit Γ (`tt) ⊤           =  yes ⊢tt
  --end
  ...
  ...   | yes A≡B             =  yes (⊢↑ ⊢M A≡B)
  inherit Γ (`case⊤ L [tt⇒ M ]) T with synthesize Γ L
  ... | no ¬∃                 = no (λ{ (⊢case⊤ ⊢L ⊢M) → ¬∃ ⟨ _ , ⊢L ⟩ })
  ... | yes ⟨ a ⇒ b , case ⟩  = no (λ{ (⊢case⊤ ⊢L ⊢M) → ⊤≢⇒ (uniq-↑ ⊢L case)})
  ... | yes ⟨ `ℕ , case ⟩     = no (λ{ (⊢case⊤ ⊢L ⊢M) → ⊤≢ℕ (uniq-↑ ⊢L case)}) 
  ... | yes ⟨ ⊤ , case ⟩ with inherit Γ M T
  ...     | yes ⊢M            = yes (⊢case⊤ case ⊢M)
  ...     | no  ⊢K            = no λ{ (⊢case⊤ _ ⊢M) → ⊢K ⊢M }
