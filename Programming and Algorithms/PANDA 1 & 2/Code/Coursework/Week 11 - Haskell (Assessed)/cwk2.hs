...
split2::Int->[Char]->String -- Splits a list of characters dependent on the place of the int. Every time that Int multiple is reached in teh list of characters.
split2 n [] = []
...
funcinput (-1) = []
funcinput x = [(triangle y) | y<-[0 .. x]] 

-- IMAGE GENERATION

genfinal::[Integer]->[Integer]->String --ll len2ndlast
genfinal [] [] = []
genfinal [x] _ = show x 
...
genrow::[Integer]->String->Bool->String
genrow [] [] _ = []
--genrow [] [x:xs] = '.':genrow [] xs True
...
--pasHelp s y = mayerrow ++ "\n" ++ (pasHelp mayerrow (y-1))
